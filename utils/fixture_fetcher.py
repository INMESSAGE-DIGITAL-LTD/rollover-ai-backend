...
def _generate_match_options(fixtures, predictor, stats_calculator, sm_stats=None, af_stats=None, free_mode=False):
    ...
    for fix in fixtures:
        ...
        # League DNA — boost/dampen market probabilities based on league scoring profile
        league_tendency = None
        _league_str = fix.get('league_name', '')
        if _league_str:
            try:
                from utils.league_dna import get_league_dna as _get_league_dna
                _dna = _get_league_dna(_league_str)
                if _dna.profile != 'balanced':
                    league_tendency = {
                        'over_boost':    1.0 + max(0.0, _dna.over_adjustment),
                        'over_dampen':   1.0 + min(0.0, _dna.over_adjustment),
                        'under_boost':   1.0 + max(0.0, _dna.under_adjustment),
                        'under_dampen':  1.0 + min(0.0, _dna.under_adjustment),
                        'result_dampen': 1.0 + min(0.0, _dna.result_adjustment),
                        'home_boost':    1.0 + max(0.0, _dna.result_adjustment),
                    }
            except Exception:
                pass
