...
class PredictionAppApiController extends Controller
{
    public function settings(Request $request)
    {
        ...
        $not_allowed_keys = [
            ...
            'site_title',
            'prediction_app_android_onesignal_app_id',
            'prediction_app_android_onesignal_api_key',
            'prediction_app_android_firebase_server_key',
            'prediction_app_android_firebase_topics',
            'prediction_app_ios_onesignal_app_id',
            'prediction_app_ios_onesignal_api_key',
            'prediction_app_ios_firebase_server_key',
            'prediction_app_ios_firebase_topics',
        ...
        $settings = Cache::rememberForever("prediction_app_settings", function () use ($request, $platform, $hidden_platform, $not_allowed_keys){
            $settings = Setting::select("*", \DB::raw("REPLACE(REPLACE(name, '{$platform}_', ''), 'prediction_app_', '') as name"))
                                ->where("name", "like", "%prediction_app%")
    ...
    public function news(Request $request)
    {
        ...
        $seconds = 60 * 20;
        $news = Cache::remember("prediction_app_news_{$page}", $seconds, function () use ($baseUrl){
            $html = self::loadFromUrl($baseUrl);
            if($html == null){
                return [];
            }
    ...
    public function prediction($html){
        ...
        $headToHeadDiv = $body->find('div .card.mb-2.py-1.card-body.rounded-3.border-0');
